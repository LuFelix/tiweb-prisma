...
public class ControlaOperacao {
	...
	public String criaCodiOp() {
		...
		String codiOperacao = String.valueOf(daoOp.consultaUltimo())
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!op.equals(null) & daoOp.alterar(op)) {
					...
					FrameInicial.setPainelVisualiza(
							new PainelOperacao(op.getCodiAtivo()));
	...
	public JTable tabela() {
		...
		for (int i = 0; i < dados.size(); i++) {
			Object linha[] = {dados.get(i).getCodiAtivo(),
					String.valueOf(dados.get(i).getValorPapel()),
	...
	public JTable pesqNomeTabela(String nome) {
		...
		for (int i = 0; i < arrayOp.size(); i++) {
			Object linha[] = {arrayOp.get(i).getCodiAtivo(),
					String.valueOf(arrayOp.get(i).getValorPapel()),
