...
public class PainelPedidos extends JPanel {
	...
	public PainelPedidos(String nome) {
		...
		tabVisualiza.addTab("Ítens", scrP01);
		tabVisualiza.addTab("Cond. de Pagamento", scrP02);
	...
	// }
	public static BigDecimal getQuantInformada() {
		String in = JOptionPane.showInputDialog(null, "QUANTIDADE");
		BigDecimal quant = new BigDecimal(in.replace(",", "."));
	...
	public static void adicionaItem(Produto prod, String nomeTabela) {
		...
		if (nomeTabela.equals("Selecionar Tabela")) {
			...
			boolean respUltPreco = contProd.consultaUltimoPreco(prod,
					nomeTabela);
			if (respUltPreco != false) {
				String in = JOptionPane.showInputDialog(null, "QUANTIDADE");
				if (in == null | in != "") {

				} else {
					quant = new BigDecimal(in.replace(",", "."));
				}
				if (quant.compareTo(new BigDecimal(0)) < 0 | quant.equals(null)
						| quant.equals("")) {
					JOptionPane.showMessageDialog(null,
				...
				int respAltPreco = JOptionPane.showConfirmDialog(null,
						"Deseja alterar o preço para esse produto e inserir?");
				if (respAltPreco == 0) {
					contProd.editarPreco(prod);
					contProd.consultaUltimoPreco(prod, nomeTabela);
					prod.setQuantMovimento(getQuantInformada());
					pedi.getItensProduto().add(prod);
					contPedi.alterarQuantProd(pedi, prod);
					daoProdEstoque.novoMovProdEstoque("Padrao", null,
							prod.getCodi_prod_1(), pedi.getQuantItens(),
							pedi.getCodiPedi(), "Sai");
					System.out.println(
							">>>>>>>>>>>>>>>>>>>>>>>>  PainelPedido.adicionarItem linha 434 "
									+ prod.getPrec_prod_1());
					atualizaTabelaItensPedido(pedi);
				}
	...
	public static void adicionaBaixaTitReceber(CondPagamento condPag) {
		...
		if (entrada != null) {
			...
			baixa.setCodiLanc(pedi.getCodiPedi());
			baixa.setCodiPedido(pedi.getCodiPedi());
	...
	public static void editaBaixaTitReceber(Lancamento baixa) {
		...
		if (entrada != null & entrada != "") {
			...
			JOptionPane.showMessageDialog(null,
					"Valor inválido. Favor tentar novamente!", "Erro",
					JOptionPane.ERROR_MESSAGE);
		}
	...
	public static JTable atualizaTabelaItensPedido(final Pedido pedi) {
		System.out.println("PainelPedidos.atualizaTabelaItensPedido");
	...
	public static Pedido leCampos() {
		...
		if (dtPkRegis.getModel().getValue() != null) {
			GregorianCalendar data = (GregorianCalendar) dtPkRegis.getModel()
					.getValue();
			pedi.setDataHoraPedi(Timestamp.from(data.toInstant()));
	...
	public static void carregarCampos(Pedido pedi) {
		if (!pedi.equals(null)) {
			...
			DateTime dtRegis = new DateTime(pedi.getDataHoraPedi());
			dtPkRegis.getModel().setDate(dtRegis.getYear(),
					dtRegis.getMonthOfYear() - 1, dtRegis.getDayOfMonth());
			txtFCodigoPedi.setText(pedi.getCodiPedi());
